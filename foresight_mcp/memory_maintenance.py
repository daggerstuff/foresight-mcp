...
class MemoryMaintenanceJob:
    ...
    def _run_consolidate(
        ...
        for cand in high_confidence:
            try:
                self._auto_consolidate(conn, cand)
    ...
    def _auto_consolidate(self, conn: Any, cand: DuplicateCandidate) -> None:
        ...
        for mid in other_ids:
            r = conn.execute("SELECT content FROM memories WHERE id = ?", (mid,)).fetchone()
            if r and r["content"]:
                additional_content.append(r["content"])
                existing_synth.add(mid)
        ...
        for mid in other_ids:
            ...
            if cursor:
                conn.execute(
                    "UPDATE memories SET is_ghost = 1, gist = ?, synthesized_from = ? WHERE id = ?",
                    (
                        (cursor["content"] or "")[:200],
                        str(list(existing_synth) + [mid]),
                        mid,
                    ),
                )
