...
class MemoryOptions(BaseModel):
    ...
    retention: str = Field(
        default="short_term",
        description="Retention policy: ephemeral, short_term, long_term, or permanent",
    )
    ...
    emotional_context: dict[str, Any] | None = Field(
        default=None,
        description="Emotional metadata (valence, arousal, dominance, primary_emotion, intensity)",
    )
    metrics: dict[str, Any] | None = Field(
        default=None,
        description="Empathy metrics (reciprocity, validation_accuracy, resistance_level)",
    )
...
def _handle_memory_update(uid: str, tenant_id: str, options: MemoryAction) -> str:
    ...
    conn.execute(
        f"UPDATE memories SET {', '.join(updates_list)} WHERE id = ? AND user_id = ? AND tenant_id = ?",
        values,
    )
    conn.commit()
    conn.close()
    ...
    if opts.include_cache_metrics:
        ...
        try:
            ...
            result["cache_metrics"] = {
                "narrative_cache": {
                    "type": "persistent",
                    "error": "Unable to retrieve narrative cache metrics",
                },
                "reflection_narrative_in_process_cache": {
                    "type": "in_process",
                    "size": len(_reflection_narrative_cache),
