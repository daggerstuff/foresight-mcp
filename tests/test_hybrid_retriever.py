...
class TestKeywordSearch:
    ...
    def test_finds_matching_memories(self, test_db):
        ...
        result = retriever.search("anxiety", "test_user", limit=5, use_graph=False, use_temporal=False)
    ...
    def test_no_results_for_nonexistent(self, test_db):
        retriever = HybridRetriever(test_db)
        result = retriever.search("xyznonexistent", "test_user", limit=5, use_graph=False, use_temporal=False)

        assert len(result.results) == 0

    def test_multi_term_query(self, test_db):
        retriever = HybridRetriever(test_db)
        result = retriever.search("feeling anxious", "test_user", limit=5, use_graph=False, use_temporal=False)
...
class TestGraphSearch:
    ...
    def test_finds_memories_via_entity(self, test_db):
        retriever = HybridRetriever(test_db)
        result = retriever.search("anxiety", "test_user", limit=5, use_keyword=False, use_temporal=False)
    ...
    def test_no_graph_results_for_unknown_entity(self, test_db):
        retriever = HybridRetriever(test_db)
        result = retriever.search("xyztity", "test_user", limit=5, use_keyword=False, use_temporal=False)
...
class TestTemporalSearch:
    ...
    def test_ranks_by_importance_and_recency(self, test_db):
        retriever = HybridRetriever(test_db)
        result = retriever.search("", "test_user", limit=5, use_keyword=False, use_graph=False, use_temporal=True)
    ...
    def test_respects_min_importance(self, test_db):
        retriever = HybridRetriever(test_db)
        result = retriever.search("", "test_user", limit=5, min_importance=0.8, use_keyword=False, use_graph=False, use_temporal=True)
...
class TestHybridFusion:
    ...
    def test_signal_counts(self, test_db):
        retriever = HybridRetriever(test_db)
        result = retriever.search("anxiety", "test_user", limit=5)
    ...
    def test_all_signals_disabled(self, test_db):
        retriever = HybridRetriever(test_db)
        result = retriever.search("anxiety", "test_user", limit=5, use_keyword=False, use_graph=False, use_temporal=False)
...
class TestRRF:
    ...
    def test_rrf_prefers_multi_signal(self):
        ...
        result = retriever._reciprocal_rank_fusion(keyword, graph, temporal)
